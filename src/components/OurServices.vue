<template>
     <!-- For demo purpose -->
        <div class="container py-5">
          <div class="row text-center text-white">
              <div class="col-lg-8 mx-auto">
                  <h1 class="display-4">OUR SERVICES</h1>
                  <!-- <p class="lead mb-0">Using Bootstrap 4 grid and utilities, create a nice team page.</p>
                  <p class="lead">Snippet by<a href="https://bootstrapious.com/snippets" class="text-white"> 
                      <u>Bootstrapious</u></a>
                  </p> -->
              </div>
          </div>
        </div><!-- End -->
        
        
        <div class="container">
          <div class="row text-center">
        
              <!-- CLEARING AND FORWARDING item -->
              <div class="col-xl-4 col-sm-6 mb-5">
                  <div class=" h-100 rounded shadow py-5 px-4">
                  <img src="../assets/images/icons/clear&forward1.png" 
                       alt="" width="200" class="img-fluid rounded-circle mb-3">
                       <h5 class="mb-0" style="color: rgb(32, 32, 98);">CLEARING AND FORWARDING</h5>
                       <!-- <span class="small text-uppercase text-muted">CEO - Founder</span> -->
                       <div class="card-body pt-2" style="color:aliceblue; font-size:large; "> <p> We quarantee timel deliver o sea cargo bv oftering suitable vessels. ensuring etticient 
                       handling from the origin countrv to the destination</p> </div>
                  </div>
              </div><!-- End -->
        
              <!-- CARGO TRACKING item -->
              <div class="col-xl-4 col-sm-6 mb-5">
                  <div class=" h-100 rounded shadow py-5 px-4">
                    <!-- <svg xmlns="http://www.w3.org/2000/svg" width="100" height="80" fill="currentColor" class="bi bi-truck-flatbed rounded-circle" viewBox="0 0 16 16">
                        <path d="M11.5 4a.5.5 0 0 1 .5.5V5h1.02a1.5 1.5 0 0 1 1.17.563l1.481 1.85a1.5 1.5 0 0 1 .329.938V10.5a1.5 1.5 0 0 1-1.5 1.5H14a2 2 0 1 1-4 0H5a2 2 0 1 1-4 0 1 1 0 0 1-1-1v-1h11V4.5a.5.5 0 0 1 .5-.5zM3 11a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm9 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm1.732 0h.768a.5.5 0 0 0 .5-.5V8.35a.5.5 0 0 0-.11-.312l-1.48-1.85A.5.5 0 0 0 13.02 6H12v4a2 2 0 0 1 1.732 1z"/>
                      </svg> -->
                        <img src="../assets/images/icons/cargotrack2.png" 
                        alt="" width="200"  class="img-fluid rounded-circle mb-3 ">
                        <h5 class="mb-0 pt-1" style="color: rgb(32, 32, 98)">CARGO TRACKING</h5>
                        <!-- <span class="small text-uppercase 
                        text-muted">CEO - Founder</span> -->
                        <div class="card-body pt-2" style="color:aliceblue"> <p>ve've developea a user-triendly ana robust 
                            shipment management intertace that empowers our customers to track 
                            their cargo's journev from the point of loading to its final destination</p> 
                        </div>
                  </div>
              </div><!-- End -->
        
              <!-- ROAD FREIGHT item -->
              <div class="col-xl-4 col-sm-6 mb-5">
                  <div class=" h-100 rounded shadow py-5 px-4">
                    <img src="../assets/images/icons/roadfreight3.png" 
                    alt="" width="200"  height="200"  class=" img-fluid rounded-circle ">
                      <h5 class="mb-0 pt-3" style="color:rgb(32, 32, 98)">ROAD FREIGHT</h5>
                      <!-- <span class="small text-uppercase text-muted">
                        CEO - Founder</span> --> 
                      <div class="card-body pt-2" style="color:aliceblue">
                        <p>  We offer comprenensive door-to-door ground freight services for both 
                             domestic and international shipments, catering to both full truck load
                             and less than a truck load full truck load and less than a truck load
                             recuirements
                        </p>
                      </div>
                  </div>
              </div><!-- End -->
          </div>
        </div>

        <div class="container py-5">
            <div class="row text-center text-white">
                <div class="col-lg-8 mx-auto">
                    <h1 class="display-4"> WHY CHOOSE US?</h1>
                </div>
            </div>
        </div><!-- End -->

        <div class="container">
            <div class="row text-center">
                <div class="col-xl-6 col-sm-4 mb-5 bg-red">
                    <div class="">
                    
                        <div class=" text-center display-6">
                        <p>  we are dedicated to providing added value to our 
                            customers by using cutting-edge technology into our work.
                            That is why the goal for continuous improvement is the 
                            engine genie ourtransoortation coman.
                        </p>
                        </div>
                    </div>
                </div><!-- End -->

                <div class="col-xl-2 col-sm-4 mb-5 "></div>
                

                <div class="col-xl-4 col-sm-4 mb-5 pt-0 ">
                    
                    <!-- <div class="card w-100 ">
                        <div class="card-body">
                        <h5 class="card-title">Head Quaters</h5>
                        </div>
                        <div class="ratio ratio-4x3">
                            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3961.4496508844227!2d39.264686985377715!3d-6.836569275528865!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x185c4b4b5756532d%3A0x1b447e617e708702!2sOverland%20carriers%20%26%20E.%20Awadh%20company!5e0!3m2!1spl!2spl!4v1698931297389!5m2!1spl!2spl" width="600" height="450" style="border:0;"  loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                        </div>
                    </div>  -->
                    <video width="500" height="320" src="../assets/videos/mapanimated.mp4">
                    </video>
                </div>
            </div><!-- End -->
        </div>

</template>

<style>

body {
    background: #e8cbc0;
    background: -webkit-linear-gradient(to right, #e8cbc0, #636fa4);
    background: linear-gradient(to right, #e8cbc0, #636fa4);
    min-height: 100vh;
  }
  
  .social-link {
    width: 30px;
    height: 30px;
    border: 1px solid #ddd;
    display: flex;
    align-items: center;
    justify-content: center;
    color: #666;
    border-radius: 50%;
    transition: all 0.3s;
    font-size: 0.9rem;
  }
  
  .social-link:hover, .social-link:focus {
    background: #ddd;
    text-decoration: none;
    color: #555;
  }</style>